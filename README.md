...
아래 검증일과 범위는 로컬 실행 기준입니다. 9월 5일 개선 내용과 재현 근거는 [프로젝트 개선 기록](docs/project-improvements.md)에 정리했습니다. CI의 최신 실행 상태는 상단 배지에서 확인할 수 있습니다.

| 검증 영역             |          결과 | 확인 범위                                                                |
| --------------------- | ------------: | ------------------------------------------------------------------------ |
| 단위·코드 계약 테스트 |  129/129 통과 | 9월 5일: 기존 계약, 신고 상태 판정, 승인율, 페이지 조회와 근거 표시      |
| Chromium 사용자 흐름  |    35/35 통과 | 9월 5일: 핵심 흐름, 20회 대화, 업로드 재시도, 모바일, 접근성과 시각 회귀 |
| PostgreSQL 운영 계약  |    31/31 통과 | 9월 5일: 기존 계약, 최신 실행과 조서 연결, 페이지 조회, 전체 감사 검색   |
| 결정론적 AI 평가      |    45/45 통과 | 9월 5일: 검색, 인용 무결성, 답변 보류, 프롬프트 인젝션과 조직 간 누출    |
| 선택한 실모델 설정    |    40/40 통과 | 기존 실모델 평가 기록, 이번 개선에서는 유료 모델을 다시 호출하지 않음    |
| 역할별 시연 녹화      |      2/2 통과 | 8월 녹화 기록, 이번 UI 개선 후 영상은 다시 제작하지 않음                 |
| 배포 품질 게이트      | 5개 작업 구성 | 전체 검증, 브라우저, DB, 컨테이너 3종과 Terraform                        |

Linux Chromium에서도 대시보드와 모바일 화면 3종의 접근성 및 시각 회귀 검사를 추가로 통과했습니다.
...
데모 업로드는 대기열 등록까지만 진행합니다. 실제 파일 처리와 상태 전환에는 DB와 워커 구성이 필요합니다. AI 실행은 현재 질문 한 건을 기준으로 하며 이전 대화가 자동으로 모델에 전달되지는 않습니다. 후속 질문에는 필요한 조건을 함께 적어 주세요.
...
E2E는 기본적으로 별도 서버를 시작합니다. 고정된 테스트 데이터로 이미 실행한 서버를 이용할 때만 `PLAYWRIGHT_PORT=3119 PLAYWRIGHT_REUSE_SERVER=true npm run test:e2e`를 사용합니다. 시각 기준선은 운영체제별로 관리하며 의도한 화면 변경을 검수한 뒤 갱신합니다.
...
- [9월 5일 프로젝트 개선 기록](docs/project-improvements.md)
...
- 이 프로젝트는 특정 회계법인이나 기업의 공식 제품 또는 보증물이 아닙니다.
