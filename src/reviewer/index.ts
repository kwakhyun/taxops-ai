...
const workpaperSchema = z.strictObject({
...
async function issueWorkpaperTokens(input: z.infer<typeof tokenRequestSchema>) {
...
function response(
  ...
  status: number,
  value: unknown,
  metadata: { method: string; path: string; timestamp: string; nonce: string },
) {
...
function requestMetadata(request: IncomingMessage, path: string) {
...
async function verifiedIdentity(input: {
  ...
  expectedActor: z.infer<typeof actorSchema>;
}) {
...
async function decideWorkpaper(input: z.infer<typeof workpaperSchema>) {
  const identity = await verifiedIdentity(input);
  return sql.begin(async (transaction) => {
    await transaction`SELECT set_config('app.tenant_id', ${identity.tenantId}, true)`;
    const actor = await resolveReviewer(
...
async function decideEvidence(input: z.infer<typeof evidenceSchema>) {
  const identity = await verifiedIdentity(input);
  return sql.begin(async (transaction) => {
    await transaction`SELECT set_config('app.tenant_id', ${identity.tenantId}, true)`;
    const actor = await resolveReviewer(
...
const server = createServer(async (request, target) => {
  ...
  let metadata:
    | { method: string; path: string; timestamp: string; nonce: string }
    | undefined;
  try {
    if (
    ...
    const encrypted = await readBody(request);
    ...
    const result =
      requestUrl.pathname === "/v1/tokens/workpapers"
        ? await issueWorkpaperTokens(tokenRequestSchema.parse(opened))
        : requestUrl.pathname === "/v1/decisions/workpapers"
          ? await decideWorkpaper(workpaperSchema.parse(opened))
          : await decideEvidence(evidenceSchema.parse(opened));
    if (!result) {
      response(target, 409, { ok: false, code: "DECISION_CONFLICT" }, metadata);
      return;
    }
    log("review.decision_succeeded", {
    ...
    response(target, 200, result, metadata);
  } catch (error) {
    log("review.decision_failed", {
      errorCode: error instanceof Error ? error.name : "UNKNOWN",
      outcome: "FAILED",
    });
    if (metadata) {
      response(target, 400, { ok: false, code: "REQUEST_REJECTED" }, metadata);
    } else {
...
server.listen(port, "0.0.0.0", () =>
  log("review.service_started", { port, outcome: "SUCCESS" }),
);

async function shutdown(signal: string) {
  log("review.service_stopping", { signal });
  server.close();
