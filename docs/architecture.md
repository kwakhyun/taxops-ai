...
기준선 갱신은 의도한 UI 변경을 검수할 때만 수행합니다. Mac에서는 `npm run test:e2e:update`를 사용하고, Linux에서는 GitHub Actions의 `visual-baseline-candidates`를 수동 실행합니다. Linux 워크플로는 세 후보를 생성한 뒤 갱신 옵션 없이 전체 E2E를 다시 실행합니다. 통과한 실행의 `visual-baselines-linux` 이미지를 직접 검수해 `linux` 디렉터리에 반영하며, 단순히 검사 실패를 없애기 위해 후보를 승인하지 않습니다. 운영체제별 기준선을 관리하는 이유는 [Playwright 시각 비교 안내](https://playwright.dev/docs/test-snapshots)를 참고합니다.
...
세무 업무와 감사 로그는 서버에서 검색과 페이지 조회를 수행합니다. 업무 쿼리는 페이지에 해당하는 업무를 먼저 선택하고 해당 자료만 집계합니다. 통합 검색은 문서 집계 없이 이동용 필드만 최대 9건 반환하며, 브라우저에서 250ms 입력 지연, 요청 취소와 30초 캐시를 적용합니다. 공통 `usePagedQuery`는 검색 중 이전 결과를 숨기고 오류 재시도를 제공합니다. 큰 규모의 처리량이나 조회 지연은 아직 부하 테스트로 측정하지 않았습니다.

세션 조회는 React `cache`로 같은 서버 렌더링 요청 안에서만 중복을 제거합니다. 세션 또는 조직 결과를 요청 간 전역 캐시에 저장하지 않습니다. 감사 목록과 내보내기는 페이지 제한과 무관하게 같은 조직, 검색, 날짜 조건을 사용합니다.

최신 분석에 표시할 조서는 `workpaper_versions.provenance.runId`로 해당 실행에 연결합니다. 최신 실행이 실패했거나 산출물을 만들지 않았다면 과거 조서를 대신 표시하지 않습니다. 신고 전 점검은 등록 자료 승인, 최신 분석 검증, 해당 조서 승인, 추적 ID를 각각 판정하며 세목별 필수 서류 충족 여부는 자동 판정하지 않습니다.

색상과 타이포그래피 토큰은 `src/app/styles/tokens.css`, 공통 버튼과 카드는 `primitives.css`, 대시보드 고유 레이아웃은 CSS Module에 둡니다. 기존 전역 규칙에서 사용하지 않는 대시보드 영역과 중복 선언을 정리했습니다. 신규 화면의 고유 스타일은 전역 테마를 추가하는 대신 해당 컴포넌트에 둡니다.
