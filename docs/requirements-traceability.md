...
| DB 설계, 쿼리와 마이그레이션     | 19개 테이블, 8개 enum, pgvector/전문 검색, 복합 외래 키와 인덱스, Drizzle 스키마와 SQL 마이그레이션         | PostgreSQL 마이그레이션과 초기 데이터, RLS·역할·승인 경계 29건        | 구현                      |
...
| 파일 중심 업무 UX                | 단계별 업무 등록, 대화 내 자료 첨부, AI 근거 확인, 6단계 Engagement, 검토 승인, 감사 검색과 내보내기        | E2E 시나리오 31건, 3개 시각 기준선, 10개 화면 × 6개 뷰포트 검사       | 구현                      |
