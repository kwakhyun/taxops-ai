...
| DB 설계·쿼리·migration           | 18개 테이블·enum, pgvector/FTS, 복합 FK·index, Drizzle schema와 SQL migration           | PostgreSQL migration/seed, RLS·역할·승인 경계 24건              | 구현                      |
...
| 파일 중심 업무 UX                | 케이스→업로드→AI 근거→검토 승인→감사 흐름                                               | 7개 end-to-end 시나리오                                         | 구현                      |
...
1. Analyst가 케이스를 만들고 Reviewer를 지정합니다.
2. 위장 파일은 즉시 거부되고 유효한 자료는 quarantine과 durable queue로 이동합니다.
3. 워커가 검사와 인덱싱을 완료하고 Reviewer가 AI 근거 사용을 승인한 자료만 검색 대상이 됩니다.
4. AI는 현재 tenant와 케이스 안에서 근거를 찾고, 계산 도구를 사용해 초안을 만듭니다.
5. 별도 verifier가 인용과 주장을 확인합니다. 근거가 없으면 기권합니다.
6. Reviewer는 artifact hash에 묶인 일회성 토큰으로 승인하거나 반려합니다.
7. 모든 중요 동작은 hash-chain audit event로 남습니다.

## 검증 범위의 한계

2026-08-29 로컬 검증에서는 실제 PostgreSQL/pgvector 마이그레이션과 RLS·역할·승인 경계 계약 24건, 분리 Reviewer OIDC/HTTP 계약, MinIO·ClamAV Worker 스모크를 컨테이너에서 통과했습니다. 근거 0건, 위조된 원문 필드, 앱 역할의 lineage 직접 쓰기를 거부하고, 정상 승인이 연결된 AI 실행 상태를 함께 전이하는 시나리오를 포함합니다. 이는 로컬 통합 검증이며 AWS 계정에 Terraform을 적용했거나 실제 IdP, DLP, 의미 분류기, 문서 처리기와 연결했다는 의미는 아닙니다.
